...
Created by Kkaxh1
"""
from src.destruction import DestructionMode
import os
import sys
import signal
import argparse
...
class NoiseMatrix:
    ...
    def save_target(self, target: Dict):
        ...
        try:
            ...
            console.print(f"[red]✗ Failed to save target: {e}[/red]")
            return False
    ...
    def setup(self):
        ...
        try:
            if not check_root():
                ...
                console.print("[yellow]Please run with: sudo python3 noisematrix[/yellow]")
                sys.exit(1)
    ...
    def show_main_menu(self) -> str:
        ...
        menu_items = [
            ...
            ("6", "💀", "Destruction Mode", "🔥 SPAWN 6 TERMINALS - MAX POWER!"),
            ("7", "⚙️", "Settings", "Configure tool parameters"),
            ("8", "📚", "Help", "View documentation and help"),
            ("9", "ℹ️", "About", "About NoiseMatrix"),
            ("10", "🚪", "Exit", "Exit and cleanup")
        ]
    ...
    def display_scan_results(self):
        ...
        console.print()
        console.print("[1] 💾 Save results (add to targets)")
        console.print("[2] 🎯 Select target")
        console.print("[3] ↩️  Main menu")
        console.print()
        ...
        if choice == "1":
            # Save to JSON file AND add to saved targets
            filename = self.scanner.save_results()
            if filename:
                console.print(f"[green]✓ Results saved to {filename}[/green]")
                
                # Also save each network as a target
                console.print("[yellow]Adding networks to saved targets...[/yellow]")
                saved_count = 0
                for network in self.scanner.networks[:10]:  # Save top 10
                    if network.get('bssid') and len(network['bssid']) > 10:
                        target = {
                        ...
                        if self.save_target(target):
                            saved_count += 1
                
                console.print(f"[green]✓ Added {saved_count} networks to saved targets![/green]")
            else:
                console.print("[red]✗ Failed to save results[/red]")
            input("\nPress Enter to continue...")
            return
...
[yellow]Target Information:[/yellow]
  ...
  • Encryption: [cyan]{target.get('encryption', 'Unknown')}[/cyan]
        ...
        # More aggressive packet rates
        packet_rates = {"low": 500, "medium": 1500, "high": 3000, "insane": 5000}
        target['packet_rate'] = packet_rates[intensity]
        target['duration'] = int(duration) if duration.isdigit() else 0
        
        # Show warning about aggressive mode
        if intensity == "insane":
            console.print()
            console.print("[red]⚠️  INSANE MODE: 5000 packets/sec![/red]")
            console.print("[yellow]This may crash your system if hardware is weak![/yellow]")
            console.print()
    ...
    def destruction_mode(self):
        ...
        target = self.select_target_from_list(self.scan_results)
        if not target:
            console.print("[red]No target selected![/red]")
            return
            
        console.print()
        warning = Panel(
            "[red]⚠️  DESTRUCTION MODE WARNING[/red]\n"
            "[yellow]This will open 6 terminals with simultaneous attacks\n"
            "Will use: aireplay-ng, mdk3 (x2), beacon flood, probe flood, channel hopper\n"
            "[red]This is EXTREMELY aggressive and may crash your system![/red]\n"
            "[yellow]Only use against networks you own or have permission to test![/yellow]",
            border_style="red"
        )
        console.print(warning)
        console.print()
        
        if Confirm.ask("[red]Are you sure you want to launch DESTRUCTION MODE?[/red]"):
            destruction = DestructionMode(self.interface, self.config)
            channel = int(target.get('channel', 1))
            bssid = target.get('bssid')
            
            console.print(f"[cyan]Target: {bssid} | Channel: {channel}[/cyan]")
            console.print("[yellow]Launching in 3 seconds...[/yellow]")
            time.sleep(3)
            
            destruction.launch_destruction(bssid, channel)        
...
• [dim]Low (500 pkts/s)[/dim] - Basic testing
• [yellow]Medium (1500 pkts/s)[/yellow] - Balanced performance
• [red]High (3000 pkts/s)[/red] - Aggressive disruption
• [red blink]INSANE (5000 pkts/s)[/red blink] - Maximum power!
...
• MAC Spoofing
• Channel Hopping
• INSANE attack mode (5000 pkts/s)
